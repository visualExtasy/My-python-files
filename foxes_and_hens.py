...
def superior1(A, B=take5):
    "Does strategy A have a higher average score than B, by more than 1.5 point?"
    return  'My: ', average_score(A), 'take5: ', average_score(B) 
...
def strategy(state):
    ...
    if cards.count('F') == 0:
        return 'wait'
    elif yard < 1.0*cards.count('H')/cards.count('F')-2.5:
        return 'wait'
    else:
        return 'gather'
